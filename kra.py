...
def get_kra_token():
    ...
    try:
        ...
        if response is not None:
            try:
                error_details = response.json().get('errorMessage', error_details)
            except:
                pass
        raise Exception(f"Error getting KRA token: {error_details}")
...
if __name__ == '__main__':
    ...
    # Update module-level variables by modifying the current module's namespace
    current_module = sys.modules[__name__]
    current_module.KRA_TOKEN_ENDPOINT = os.getenv("KRA_TOKEN_ENDPOINT", "")
    current_module.KRA_ERITS_ENDPOINT = os.getenv("KRA_ERITS_ENDPOINT", "")
    current_module.KRA_CLIENT_ID = os.getenv("KRA_CLIENT_ID", "")
    current_module.KRA_CLIENT_SECRET = os.getenv("KRA_CLIENT_SECRET", "")

    print("=" * 70)
    print("KRA TAX INTEGRATION TEST SUITE")
    print("=" * 70)
    print(f"Token Endpoint: {KRA_TOKEN_ENDPOINT}")
    print(f"eRITS Endpoint: {KRA_ERITS_ENDPOINT}")
    print(f"Client ID: {KRA_CLIENT_ID[:10]}..." if KRA_CLIENT_ID else "Client ID: Not set")
    print("=" * 70)
    
    print("\n--- 1. Testing GavaConnect Authentication (get_kra_token) ---")
